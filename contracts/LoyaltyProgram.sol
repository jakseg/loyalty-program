...
contract LoyaltyRewardsToken is Initializable, ERC721Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    // Track user redemptions per action (exclusive rewards)
    ...
    // Events
    event TicketRedeemed(address indexed user, bytes32 actionId, bytes32 ticketHash, uint256 tokenId, string tier);
    ...
    // Main redemption function with proper validation
    function redeemTicket(bytes32 actionId, bytes memory signature) public returns (uint256) {
        // Validate action exists and is active
        require(bytes(rewardableActions[actionId].name).length > 0, "Action does not exist");
        require(rewardableActions[actionId].isActive, "Action not active");
        
        // Ensure exclusive redemption per user per action
        require(!userActionRedeemed[actionId][msg.sender], "Action already redeemed by user");
        ...
        // Mark ticket as used and action as redeemed by user
        usedTickets[messageHash] = true;
        userActionRedeemed[actionId][msg.sender] = true;
        
        // Mint token with predefined tier
        uint256 tokenId = _nextTokenId++;
        string memory tier = rewardableActions[actionId].tier;
        _tokenTiers[tokenId] = tier;
        
        _safeMint(msg.sender, tokenId);
        
        emit TicketRedeemed(msg.sender, actionId, messageHash, tokenId, tier);
    ...
    function isActionRedeemableByUser(bytes32 actionId, address user) public view returns (bool) {
        return rewardableActions[actionId].isActive && 
               !userActionRedeemed[actionId][user] &&
               bytes(rewardableActions[actionId].name).length > 0;
    }
